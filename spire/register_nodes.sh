...
docker-compose exec spire-server sh -c "cd /opt/spire && ./spire-server entry create -parentID spiffe://example.org/donutsalon-1 -spiffeID spiffe://example.org/backend1 -selector unix:uid:0 -ttl 120"
docker-compose exec spire-server sh -c "cd /opt/spire && ./spire-server entry create -parentID spiffe://example.org/donutsalon-2 -spiffeID spiffe://example.org/backend2 -selector unix:uid:0 -ttl 120"
docker-compose exec spire-server sh -c "cd /opt/spire && ./spire-server entry create -parentID spiffe://example.org/donutsalon-3 -spiffeID spiffe://example.org/backend3 -selector unix:uid:0 -ttl 120"
docker-compose exec spire-server sh -c "cd /opt/spire && ./spire-server entry create -parentID spiffe://example.org/front-envoy -spiffeID spiffe://example.org/frontend -selector unix:uid:0 -ttl 120"
